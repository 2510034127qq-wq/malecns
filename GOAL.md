...
如果任务上下文似乎不完整，或在中断后恢复工作：

1. 完整阅读 `AGENTS.md` 和 `GOAL.md`。
2. 检查 `git status`、`git diff` 以及最近的 git 历史。
3. 在做出任何更改之前，检查现有源代码、测试、生成的清单、基准测试报告、日志和检查点。
4. 确定在完整 MaleCNS 数据集上实际已实现并验证的内容。
5. 识别未完成/正在运行/失败的阶段以及下一步的具体行动。
6. 从最新的有效产物/检查点恢复。不要重启已完成的多小时预处理或全数据运行，除非其输出无效。
7. 切勿从旧的对话摘要中推断完成情况；必须从文件和测试/基准测试结果中核实。
